...
export default function Navbar() {
    ...
    const baseMobile = 'block text-lg font-medium transition-colors';
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex h-16 items-center justify-between">
                        ...
                        <nav className="hidden md:flex items-center gap-8">
                            ...
                            <Link className={`${baseDesktop} ${isActive('#') ? 'text-primary' : inactiveDesktop}`} href="#">Бидний тухай</Link>
            ...
            <div className={`fixed inset-0 z-50 transition-opacity ${open ? 'opacity-100 visible' : 'opacity-0 invisible'}`} aria-hidden={!open}>
                ...
                <aside className={`absolute top-0 right-0 h-full w-72 bg-background-dark/95 backdrop-blur-lg shadow-2xl transform transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`} role="dialog" aria-modal="true">
                    ...
                    <nav className="p-4 space-y-4">
                        <Link className={`${baseMobile} ${isActive('/') ? 'text-primary' : inactiveMobile}`} href="#">Нүүр</Link>
                        <Link className={`${baseMobile} ${isActive('#') ? 'text-primary' : inactiveMobile}`} href="#">Бидний тухай</Link>
                        <Link className={`${baseMobile} ${isActive('#') ? 'text-primary' : inactiveMobile}`} href="#">Хичээлүүд</Link>
                        <Link className={`${baseMobile} ${isActive('#') ? 'text-primary' : inactiveMobile}`} href="#">Блог</Link>
                        <Link className={`${baseMobile} ${isActive('/contact') ? 'text-primary' : inactiveMobile}`} href="/contact">Холбоо барих</Link>
                    </nav>
                </aside>
            </div>
        </>
