                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex h-16 items-center justify-between">
                        ...
                        <nav className="hidden md:flex items-center gap-8">
                            <Link className="text-sm font-medium text-gray-300 hover:text-primary transition-colors" href="/">Нүүр</Link>
                            <Link className="text-sm font-medium text-gray-300 hover:text-primary transition-colors" href="#">Бидний тухай</Link>
                            <Link className="text-sm font-medium text-gray-300 hover:text-primary transition-colors" href="#">Хичээлүүд</Link>
                            <Link className="text-sm font-medium text-gray-300 hover:text-primary transition-colors" href="#">Блог</Link>
                            <Link className="text-sm font-medium text-gray-300 hover:text-primary transition-colors" href="/contact">Холбоо барих</Link>
                        </nav>
            ...
            <div className={`fixed inset-0 z-50 transition-opacity ${open ? 'opacity-100 visible' : 'opacity-0 invisible'}`} aria-hidden={!open}>
                ...
                <aside className={`absolute top-0 right-0 h-full w-72 bg-background-dark/95 backdrop-blur-lg shadow-2xl transform transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`} role="dialog" aria-modal="true">
                    ...
                    <nav className="p-4 space-y-4">
                        <Link className="block text-lg font-medium text-gray-200 hover:text-white" href="#">Нүүр</Link>
                        <Link className="block text-lg font-medium text-gray-200 hover:text-white" href="#">Бидний тухай</Link>
                        <Link className="block text-lg font-medium text-gray-200 hover:text-white" href="#">Хичээлүүд</Link>
                        <Link className="block text-lg font-medium text-gray-200 hover:text-white" href="#">Блог</Link>
                        <Link className="block text-lg font-medium text-gray-200 hover:text-white" href="/contact">Холбоо барих</Link>
                    </nav>
                </aside>
            </div>
        </>
